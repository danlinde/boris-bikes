...
describe Station do 
  ...
  it 'can referenced by name' do 
    ...
    it 'is stocked with the 10 bikes' do 
      station.new_stock
    ...
    it 'can update a bike to broken' do 
      station.new_stock
      station.rent_out_a_bike 
      expect(station.bike_broken).to eq 'broken'
    end
    ...
    it 'returns an available bike' do 
      station.new_stock 
      expect(station.set_user_bike).to be_true
    end
    ...
    it 'can accept a bike if space available' do 
      station.new_stock 
      expect(station.accept_bike(station.user_bike)).to be_true
    end

    it 'cannot accept a bike if no space available' do 
      station.new_stock 
      station.accept_bike(station.user_bike)
      expect(station.accept_bike(station.user_bike)).to be_false
    end
    ...
    it 'updates list of bikes to show bikes received from the garage' do
      ...
      expect(station.list_of_bikes[0].working_order).to eq 'working' 
    end
